...
\textit{Input:} iris radius $R \in \R$; function $\textrm{image}: \R^2 \rightarrow \R^3$ centered at an expected eye location.\\
\textit{Output:} vector $\mat p \in \R^2$, the precise iris center.\\
...
\begin{itemize}
\item
Angular limbus score $\textrm{limbus}(\alpha)$ is the gradient magnitude $|\nabla \textrm{image}(\mat p)|$ where $\arctan(\mat p) = \alpha$ and $|\mat p| = R$.

\item
Radial color $\textrm{mean}(r)$ is a mean value of the set $\{\textrm{image}(\mat p) \text{ for all } |\mat p| = r \}$.

\item
Angular iris score $\textrm{iris}(\alpha)$ is the weighted arithmetic mean of the set
$$\{\textrm{image}(\mat p) \text{ for all } \arctan(\mat p) = \alpha \}.$$
The weight is $\textrm{w}(\mat p) = r \cdot (W - |\textrm{image}(\mat p) - \textrm{mean}(r)|)$, clamped to zero, with $r = |\mat p|$ and $W$ being a value appropriately chosen.

\end{itemize}

The total score is defined as $\int_\alpha \textrm{limbus}(\alpha) \cdot \textrm{iris}(\alpha)$.

Note that the formula for $\textrm{mean}(r)$ has not been specified yet.
...
Skin, and most importantly the eyelids, can be often detected by simple analysis of its color.
...
Areas masked out by this detector are excluded from eye fitting.
