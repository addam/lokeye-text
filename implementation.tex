...
\textit{Output:} geometric transformation $\f T: \R^2 \to \R^2$ such that $\int_{\vec p} |\f{R}(\vec p) - \f{I}(\f{T}(\vec p))|^2$ is minimal.\footnote{
Because we do not actually intend to use integral calculus, the integral is only written in an informal way.
The full form would read: $\int_{\vec p \in \Omega} |\f{R}(\vec p) - \f{I}(\f{T}(\vec p))|^2 \f{d}\vec p$, where $\Omega$ is the image domain.
}\\
...
\todo{note that the image functions return real 3-dimensional vectors. These correspond to RGB.}
