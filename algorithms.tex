...
Firstly, we shall describe a few algorithms commonly used to solve generic mathematical problems.
...
\todo{householder reflections to bidiagonal, gradient descent}
...
\textit{Input:} function $F \in \R^n \to \R$, its gradient $\nabla F$ and initial location $\vec p \in \R^n$.\\
\textit{Output:} local minimum of $F$.\\

We approximate the function $F$ by a quadratic polynomial using its value $F(\vec p)$ and its gradient $\nabla F(\vec p)$ at initial location.
\todo {line search}

This algorithm is provided without a proof of convergence.
In fact, if supplied with a quadratic polynomial $F$, this algorithm finds the global optimum after \todo{how many} steps.
However, we intend to use this algorithm on functions of overwhelming complexity.
...
\todo{the curvature trick belongs to the footnote}
...
Although $\left\{ \mat M^{i, j} \vec p \right\}$ generate the subspace in question, there are $\frac {1} {2} n \cdot (n - 1)$ of such vectors, whileas only $n-1$ vectors are necessary to form a basis.
