...
\begin{definition}
The \textit{convolution} of matrices $\mat M \in \R^{n \times m}$ and $\mat T \in \R^{p \times q}$ is the matrix $\mat R \in \R^{|n-p| \times |m - q|}$ defined as:
$$\mat R_{i, j} = \sum_{k, l} \mat M_{k, l} \mat T_{i - k, j - l}.
$$
We write the convolution as $\mat R = \mat M \ast \mat T$.
\end{definition}

Combining the Fast Fourier Transform with a tiling scheme can calculate the convolution in $O(mn \log{pq})$ time.
