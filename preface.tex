...
For example, there are many such videos on Youtube:
...
Eye tracking seems to be a simple task once the head pose is known---it almost feels that the eyes are designed for easy tracking.
However, it is rather difficult to design a method that would generalize well, e.g., across users.
